...
class Game {
  ...
  constructor(players_list) {
    this.draw_pile = constants_1.standard_deck;
    this.players = [];
  ...
  handle_turn(player, card, position) {
    ...
    //edge case for if the card+token is fired, idk why, but it fires, regardless for now we're considering it valid, please figure it out
    if (
      card !== current_move ||
      current_move !== card + this.players[player_idx].token
    ) {
  ...
  get_player_details_by_index(index) {
